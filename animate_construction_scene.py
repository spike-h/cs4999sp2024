...
# position, rotation, focal length (xyz, xyz, f)
camera_positions = [
    [[326.92, 153.7, 938.5], [0, 28.3, 0]],
    [[51.657, 153.7, 1214.7], [0, 0, 0]],
    [[-258.833, 163.013, 1004.1], [-0.08, -24.2, 0.396]],
    [[43.22, 164.49, 518.18], [0, 0, 0]],
    [[43.22, 164.49, 518.18], [0, 0, 0]],
    [[43.22, 164.49, 518.18], [0, -45.9, 0]],
    [[43.22, 164.49, 518.18], [0, 47, 0]],
    [[43.22, 164.49, 518.18], [0, 0, 0]],
    [[43.22, 164.49, 518.18], [28.3, 0, 0]],
    [[43.22, 164.49, 518.18], [-17.2, 0, 0]],
    [[43.22, 164.49, 518.18], [0, 0, 0]],
]

camera_focals = [24,24,24,24,15,15,15,15,15,15,15]
...
def design_scene() -> dict:
    """Design the scene."""
    # Populate scene
    cfg = sim_utils.UsdFileCfg(usd_path=f"/home/pcglab/Documents/Construction Wall Stages/done_stage3.usda")
    cfg.func("/World/contructionScene", cfg)
    # -- Ground-plane
    # cfg = sim_utils.GroundPlaneCfg()
    # cfg.func("/World/defaultGroundPlane", cfg)
    # -- Lights
    cfg = sim_utils.DistantLightCfg(intensity=2000.0, color=(0.75, 0.75, 0.75))
...
def run_simulator(sim: sim_utils.SimulationContext, scene_entities: dict):
    ...
    sim_time = 0.0 - 5 # offset by 5 to make sure it starts capturing at the right time
    ...
    print(camera_targets, camera_targets.shape)
    ...
    camera_pos = torch.tensor([camera_positions[0][0]], device=sim.device)
    camera_targets = torch.tensor([camera_positions[0][1]])
    camera.set_intrinsic_matrices(camera.data.intrinsic_matrices.cpu().data.numpy(), focal_length=camera_focals[0])
    camera.set_world_poses(camera_pos, camera_targets)

    # # These orientations are in ROS-convention, and will position the cameras to view the origin
    # camera_orientations = torch.tensor(  # noqa: F841
    #     [-0.1759, 0.3399, 0.8205, -0.4247], device=sim.device
    # )
    ...
    # Simulate physics
    while simulation_app.is_running() and sim_time < 10:
        # Step simulation
        sim.step()
        # Update camera data
        ...
        #move camera position ad orientaiton
        # We switch to the next keyframe every second
        # we run the simulation for dt = 0.01 so we multiply the difference between the frames by this amount then add to original
        print(camera.data.pos_w)
        last_pos = np.array(camera_positions[int(sim_time)])
        next_pos = np.array(camera_positions[int(sim_time)+1])
        differences = np.subtract(next_pos, last_pos)
        last_focal = camera_focals[int(sim_time)]
        focal_diff = last_focal - camera_focals[int(sim_time)+1]

        camera_pos = torch.tensor([np.add(differences[0] * (sim_time - int(sim_time)), last_pos[0])], device=sim.device)
        camera_targets = torch.tensor([np.add(differences[1] * (sim_time - int(sim_time)), last_pos[1])], device=sim.device)
        camera.set_intrinsic_matrices(camera.data.intrinsic_matrices.cpu().data.numpy(), focal_length=focal_diff * (sim_time - int(sim_time)) + last_focal)
        camera.set_world_poses(camera_pos, camera_targets)
        ...
        if args_cli.save:
            ...
            print(single_cam_info, "ASKJFBKJASF")
...
def main():
    ...
    sim_cfg = sim_utils.SimulationCfg(device="cpu" if args_cli.cpu else "cuda", dt=.01, substeps=1)
    sim = sim_utils.SimulationContext(sim_cfg)
    # Set main camera
    sim.set_camera_view([2.5, 2.5, 2.5], [0.0, 0.0, 0.0])

    # setup the camera angles as quaternions
    for i, (pos, rot) in enumerate(camera_positions):
        rot[1] = -1 * (rot[1] - 180) # orient stupid
        rot[2] = rot[2] - 180
        new_rot = [torch.tensor(math.radians(angle), device=sim.device) for angle in rot]
        new_rot = quat_from_euler_xyz(new_rot[0], new_rot[1], new_rot[2])
        camera_positions[i][1] = new_rot.cpu().numpy()
        camera_positions[i][0] = np.array(camera_positions[i][0])
